...
baseDir = '/NAS_II/Projects/pboord/Projects/EEG-fMRI/Data/EEG_FMRI_002/EEG_FMRI_002/PART1';
...
delayPoints = 0:1:8; % sec
winLen = fs*tr;
absSpecRatio = zeros(winLen,numel(delayPoints));
binStart = 8;
binEnd = 126;
posRoi = restTrRoi>=0;
negRoi = restTrRoi<0;
win = hamming(winLen,'periodic');
for delayIdx = 1:numel(delayPoints)
    hrDelaySec = delayPoints(delayIdx); % sec
    ...
    posFft = getEpochFft(delayedEegRoi,winLen,nrRestTr,nrRoi,posRoi,win);
    negFft = getEpochFft(delayedEegRoi,winLen,nrRestTr,nrRoi,negRoi,win);
    absSpecRatio(:,delayIdx) = mean(abs(posFft),2)./mean(abs(negFft),2);
    figure('WindowStyle','docked'),plot([mean(abs(posFft(binStart:binEnd,:)),2),mean(abs(negFft(binStart:binEnd,:)),2)])
end
figure('WindowStyle','docked'),surf(log10(absSpecRatio(binStart:binEnd,:)))
figure('WindowStyle','docked'),plot([mean(abs(posFft),2),mean(abs(negFft),2)])
end
function roiFft = getEpochFft(eegRoi,winLen,nrRestTr,nrRoi,signRoi,win)
