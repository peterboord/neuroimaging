...
% constants
mag1S=MRIread(fullfile(funcDir,session,'mocoBetHpf.feat','pkErpMag.nii'));
...
voxNear=angle(repmat(exp(1i*phaseNear(1,:)),nrNear-1,1).*exp(-1i*phaseNear(2:end,:)));
voxNear(magNear(2:end,:)==0)=Inf;
voxNear(:,magPad(:)==0)=Inf;
closestVox=min(voxNear.*(voxNear<0),[],1);
%closestVox=voxNear.*(voxNear<0);
closestVox(isinf(closestVox))=0;
for volNr=1:nrVol
    curClosestVox=closestVox;
    curClosestVox(closestVox<-volNr*0.01)=0;
    curClosestVox=reshape(curClosestVox,padSize);
    ves(:,:,:,volNr)=curClosestVox(:,:,2:end-1);
%     disp(volNr);
%     tic
