function findCardioPeaksV1_3(session)
...
[clean4dPulseox,noise4dPulseox,~,~,slicePhasesPulseox]=retroicor(restS.vol,allCardioPeakTimes,tr);
end

% dirty data
[rmsSlice,voxMask]=getRmsSlice(restS.vol,nrMaskVoxPerSlice);

[maxCardioPkFreq,Pxx,f]=findSlicePhases4(rmsSlice,nrTr,padFactor,minCardioHz,maxCardioHz,tr,restS,slicePhasesPulseox,cardio);

[allCardioPeakTimes,Pxx,f]=calcPeakTimes(rmsSlice,nrTr,padFactor,minCardioHz,maxCardioHz,tr);
% clean data
[~,noise4d,abMag,abPhase,slicePhases]=retroicor(restS.vol,allCardioPeakTimes,tr);
...
[clean4d,noise4dEqualized,~,~,slicePhasesEqualized]=retroicor(restS.vol,allCardioPeakTimes,tr);
...
rmsSlicePulseox=getRmsSlice(clean4dPulseox,nrMaskVoxPerSlice,voxMask);
...
respMask=std(reshape(restS.vol,[],nslice,nvol),1,3);
rmsSliceRespRaw=getRmsSlice(restS.vol,nrMaskVoxPerSlice,respMask);
fftResp=fft(detrend(rmsSliceRespRaw,'constant'),nslice*padFactor);
% respMask=std(reshape(clean4d,[],nslice,nvol),1,3);
% rmsSliceRespClean=getRmsSlice(clean4d,nrMaskVoxPerSlice,respMask);
