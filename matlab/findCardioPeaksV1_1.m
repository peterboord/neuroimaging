function findCardioPeaksV1_1(session)
...
for sliceNr=1:nslice
    ...
    slicePhases=repAbPhase>=phaseBins & repAbPhase<(phaseBins+2*pi/nrBins) & repmat(~all(reshape(restS.vol(:,:,sliceNr,:)==0,[],nvol),2),1,nrBins)...
        & repmat(varMask(:,sliceNr),1,nrBins);
...
rmsSliceClean=getRmsSlice(restS.vol,nrMaskVoxPerSlice,equalizedMask);
Palias=zeros(nrTr*nslice/2+1,nslice);
for sliceNr=1:nslice
    [PxxAlias,falias]=pwelch(rmsSliceClean(sliceNr,:),nrTr*nslice,nslice,nrTr*nslice,1/tr);
    Palias(:,sliceNr)=PxxAlias;
end
figure
hold(ax1,'on');
plot(ax1,falias,mean(Palias,2),'g');
hold(ax1,'off');
[Pxx,f]=pwelch(rmsSliceClean(:),nrTr*nslice,nslice,padFactor*nrTr*nslice,nslice/tr);
hold(ax2,'on');
plot(ax2,f,Pxx,'g');
hold(ax2,'off');
...
% pulse ox
nrTr=2;
[cardio,allCardioPeakTimes,resp,allRespPeakTimes]=getPeaksFromPhysioData(fullfile(physioDir,session,'rest_cardio.txt'),fullfile(physioDir,session,'rest_resp.txt'),tr,nvol,nslice);
[clean4dPulseox,noise4d]=retroicor(restS.vol,allCardioPeakTimes,tr);
rmsSliceClean=getRmsSlice(clean4dPulseox,nrMaskVoxPerSlice,varMask);
[Pxx,f]=pwelch(rmsSliceClean(:),nrTr*nslice,nslice,padFactor*nrTr*nslice,nslice/tr);
hold(ax2,'on');
plot(ax2,f,Pxx,'r');
hold(ax2,'off');
[Pxx,f]=pwelch(resp(:),nrTr*nslice,nslice,padFactor*nrTr*nslice,nslice/tr);
subplot(nrPlots,1,7);
plot(f,Pxx);
% roi
if ~isempty(roi)
    ...
    plot(froi,Proi,'b');
    hold on
    [Proi,froi]=pwelch(squeeze(clean4d(roi(1),roi(2),roi(3),:)),nrTr*nslice,nslice,nrTr*nslice,1/tr);
    plot(froi,Proi,'g');
    [Proi,froi]=pwelch(squeeze(clean4dPulseox(roi(1),roi(2),roi(3),:)),nrTr*nslice,nslice,nrTr*nslice,1/tr);
    plot(froi,Proi,'r');
    hold off
end
...
% getRmsSlice
%%%%%%%%%%%%%%%%%%%%%%%
function [rmsSlice,varMask]=getRmsSlice(rest,nrMaskVoxPerSlice,varMask)
restSize=size(rest);
