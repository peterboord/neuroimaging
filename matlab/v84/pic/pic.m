...
if nargin < 2
    ...
    if nargin == 2
        ...
        [funcRoiLag,funcRoiLagDiff] = lag1func(imInS.vol);
    else
        ...
        [funcRoiLag,funcRoiLagDiff]=lag1func(imInS.vol.*repmat(reshape((maskS.vol>0)*1,[maskS.volsize,1]),[1,1,1,imInS.nframes]));
    end
    ...
    MRIsave(imInS,reshape(corrColumns(reshape(abs(funcRoiLagDiff),[],imInS.nframes)',reshape(-funcRoiLag,[],imInS.nframes)'),imInS.volsize),'corrAbsFcDiffPic.nii.gz',1);
