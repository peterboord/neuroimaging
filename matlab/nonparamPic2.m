function nonparamPic2(dirName,subjCodeNrStr,filePrefix,maskName,nrRand)
% from SGE_roiCorr52
if ~isdeployed
    dbstop if error
end
...
nzpHiS=MRIread('/mnt/pboord_5TB/pic/act/109003/regress/pic/nzpHi.nii.gz');
...
%     end
    ...
    seed(seedNr).nzpHi=nzpHiS.vol(x,y,z);
end
...
corrPicActInMask=zeros(nrMaskVox,1);
hiLoAngleInMask=zeros(nrMaskVox,1);
aFcWeightHiInMask=zeros(nrMaskVox,27);
aFcWeightLoInMask=zeros(nrMaskVox,27);
for neighNr=uniqueNrNeigh
    ...
    % use act >= or < 0, instead of lo/hi half
    neighNrFunc=neighNrFunc-repmat(mean(neighNrFunc,3),1,1,nf);
    curAct=repmat(mean(neighNrFunc,1),neighNr,1,1);
...
%     curAct=repmat(mean(neighNrFunc.*repmat(2*(actVoxCorr>=0)-1,1,1,nf),1),neighNr,1,1);
    
    weightedAct=neighNrFunc.*curAct;
    posInd=curAct>=0;
    posVp=sum(weightedAct.*posInd,3)./sum(posInd,3);
    negVp=sum(weightedAct.*~posInd,3)./sum(~posInd,3); % n.b. negVp inverted bec curAct<0
    posnegVp=sum((weightedAct.*posInd)+(weightedAct.*~posInd),3)/nf;
    posVp=posVp./repmat(sqrt(sum(posVp.^2,1)),neighNr,1);
    negVp=negVp./repmat(sqrt(sum(negVp.^2,1)),neighNr,1);
    posnegVp=posnegVp./repmat(sqrt(sum(posnegVp.^2,1)),neighNr,1);
%     figure('Windowstyle','docked');
%     subplot(3,2,1);v=1;plotyy(1:nf,corr(posVp(:,v),squeeze(neighNrFunc(:,v,:))),1:nf,squeeze(mean(neighNrFunc(:,v,:),1)))
%     xlabel(num2str(corr(corr(posVp(:,v),squeeze(neighNrFunc(:,v,:)))',squeeze(mean(neighNrFunc(:,v,:))))));
%     subplot(3,2,2);v=2;plotyy(1:nf,corr(posVp(:,v),squeeze(neighNrFunc(:,v,:))),1:nf,squeeze(mean(neighNrFunc(:,v,:),1)))
%     xlabel(num2str(corr(corr(posVp(:,v),squeeze(neighNrFunc(:,v,:)))',squeeze(mean(neighNrFunc(:,v,:))))));
%     subplot(3,2,3);v=1;plotyy(1:nf,corr(negVp(:,v),squeeze(neighNrFunc(:,v,:))),1:nf,squeeze(mean(neighNrFunc(:,v,:),1)))
%     xlabel(num2str(corr(corr(negVp(:,v),squeeze(neighNrFunc(:,v,:)))',squeeze(mean(neighNrFunc(:,v,:))))));
%     subplot(3,2,4);v=2;plotyy(1:nf,corr(negVp(:,v),squeeze(neighNrFunc(:,v,:))),1:nf,squeeze(mean(neighNrFunc(:,v,:),1)))
%     xlabel(num2str(corr(corr(negVp(:,v),squeeze(neighNrFunc(:,v,:)))',squeeze(mean(neighNrFunc(:,v,:))))));
%     subplot(3,2,5);v=1;plotyy(1:nf,corr(posnegVp(:,v),squeeze(neighNrFunc(:,v,:))),1:nf,squeeze(mean(neighNrFunc(:,v,:),1)))
%     xlabel(num2str(corr(corr(posnegVp(:,v),squeeze(neighNrFunc(:,v,:)))',squeeze(mean(neighNrFunc(:,v,:))))));
%     subplot(3,2,6);v=2;plotyy(1:nf,corr(posnegVp(:,v),squeeze(neighNrFunc(:,v,:))),1:nf,squeeze(mean(neighNrFunc(:,v,:),1)))
%     xlabel(num2str(corr(corr(posnegVp(:,v),squeeze(neighNrFunc(:,v,:)))',squeeze(mean(neighNrFunc(:,v,:))))));
%     figure
%     subplot(1,2,1);v=1;plot(corr(posVp(:,v),squeeze(neighNrFunc(:,v,:))),corr(negVp(:,v),squeeze(neighNrFunc(:,v,:))))
%     subplot(1,2,2);v=2;plot(corr(posVp(:,v),squeeze(neighNrFunc(:,v,:))),corr(negVp(:,v),squeeze(neighNrFunc(:,v,:))))
    ...
    neighNrFunc=neighNrFunc-repmat(mean(neighNrFunc,3),1,1,nf);
    % normalize
    %neighNrFuncNorm=permute(neighNrFunc./repmat(sqrt(sum(neighNrFunc.^2,3)),1,1,nf),[1,3,2]);
    % or not
    neighNrFuncNorm=permute(neighNrFunc,[1,3,2]);
    % 2 May 16 end
    ...
    switch timeSubset
        case 'hiAct'
            ...
            aFcWeightHi=squeeze(sum(neighNrFuncNormSorted(:,end-floor(nf/d)+1:end,:).*hiFc,2)./sum(hiFc,2));
        case 'whole'
            hiFc=repmat(reshape(sortedFc,1,[],nrVp),neighNr,1,1);
            %wrong?... hiFc=hiFc-repmat(mean(hiFc,2),1,nf,1);
            aFcWeightHi=squeeze(sum(neighNrFuncNormSorted.*hiFc,2)./sum(hiFc,2));
    end
    aFcWeightLo=-negVp;
    aFcWeightHi=posVp;
...
%     aFcWeightHi=aFcWeightHi./repmat(sqrt(sum(aFcWeightHi.^2,1)),neighNr,1);
    ...
    aFcWeightHiInMask(neighNrVox,1:neighNr)=aFcWeightHi';
    aFcWeightLoInMask(neighNrVox,1:neighNr)=aFcWeightLo';
    hiLoAngleInMask(neighNrVox)=sum(aFcWeightLo.*aFcWeightHi,1);
    %
...
picSlideInMask(neighNrVox,:)=reshape(sum(neighNrFuncNorm.*reshape(repmat(aFcWeightHi,nf,1),neighNr,[])),nf,nrVp)';
corrPicActInMask(neighNrVox,:)=corrColumns(picSlideInMask(neighNrVox,:)',fcInMask(neighNrVox,:)');
...
funcS.vol(maskInd)=hiLoAngleInMask;
funcS.vol=reshape(funcS.vol,funcS.volsize);
MRIsave(funcS,funcS.vol,fullfile(picDir,'hiLoAngle.nii.gz'),1);
% save aFcWeightHi
funcS.vol=zeros(prod(funcS.volsize),27);
funcS.vol(maskInd,:)=aFcWeightHiInMask;
funcS.vol=reshape(funcS.vol,[funcS.volsize,27]);
MRIsave(funcS,funcS.vol,fullfile(picDir,'aFcWeightHi.nii.gz'),1);
% save aFcWeightLo
funcS.vol=zeros(prod(funcS.volsize),27);
funcS.vol(maskInd,:)=aFcWeightLoInMask;
funcS.vol=reshape(funcS.vol,[funcS.volsize,27]);
MRIsave(funcS,funcS.vol,fullfile(picDir,'aFcWeightLo.nii.gz'),1);
% save corrPicAct
funcS.vol=zeros(prod(funcS.volsize),1);
funcS.vol(maskInd)=corrPicActInMask;
funcS.vol=reshape(funcS.vol,funcS.volsize);
MRIsave(funcS,funcS.vol,fullfile(picDir,'corrPicAct.nii.gz'),1);
